...
namespace cAlgo.Indicators
{
    ...
    public class NCT : Indicator
    {
        ...
        [Parameter("Font Size", DefaultValue = "Normal", Group = "Visual Customization")]
        ...
        [Parameter("Target Max Count", DefaultValue = 20, MinValue = 1, MaxValue = 500, Group = "Node Targets")]
        public int TargetMaxCount { get; set; }

        [Parameter("Pair Max Count", DefaultValue = 20, MinValue = 1, MaxValue = 500, Group = "Node Targets")]
        ...
        private void ResetState()
        {
            ...
            try { RemoveDrawings(); } catch { /* Chart may be unavailable during Initialize */ }
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(TextNodeSize))
                return 12;

            switch (TextNodeSize.Trim())
            {
                case "Huge": return 18;
        ...
        private void RegisterDensity(double price, bool isUp)
        {
        ...
        private void DrawingTargetsNode1(bool swUptrendType)
        {
            ...
            int skip = Math.Max(qualifies.Count - TargetMaxCount, 0);
            ...
            for (int q = skip; q < qualifies.Count; q++)
            {
                ...
                if (ShowDouble)
                {
                    ...
                    if (!ShouldDeleteHitTarget(price, node.IndexNode, swUptrendType))
                    {
                        ...
                        RegisterDensity(price, swUptrendType);
                    }
                }
        ...
        private void DrawingPairTargetsNode12(bool swUptrendType)
        {
            ...
            int skip = Math.Max(pairStarts.Count - PairMaxCount, 0);
            LineStyle styleDash = ParseLineStyle(TargetLineStyleName);
            LineStyle styleMin = ParseLineStyle(MinLineStyleName);
            int lastIndex = Bars.Count - 1;
            int endIdx = lastIndex + TargetGapBars;
            ...
            int width, LineStyle style, string label, Color labelColor)
        {
            ...
            long midTicks = startTime.Ticks + (endTime.Ticks - startTime.Ticks) / 2;
            var midTime = new DateTime(midTicks, startTime.Kind);
            var txt = Chart.DrawText(NextName("TgtLbl"), label, midTime, price, labelColor);
            txt.FontSize = 9;
        ...
        private void DrawDensityClusters()
        {
            ...
            var used = new bool[n];
            DateTime lastTime = Bars.OpenTimes[Bars.Count - 1];
            ...
            for (int oi = 0; oi < n; oi++)
            {
                ...
                var circ = Chart.DrawText(NextName("Dens"), "●", lastTime, midPrice, dotColor);
                circ.FontSize = 18;
